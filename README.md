...
ReClass is a multi-tenant remedial-class management application for Kenyan schools. It covers school administration, scheduling, teacher delivery and payroll, student/guardian records, billing, M-Pesa payments, SMS notifications, parent messaging, and exam results.

> **Status: BLOCKED** (audit date: 2026-07-22). The repository builds and its current unit/static contract tests pass, but it is not safe or reproducible for production. The migration chain fails from an empty database, hosted state is unverified, provider credentials are stored without the encryption promised by the UI, and the normal server data path uses a service-role client that bypasses RLS. See [FINAL_REPORT.md](FINAL_REPORT.md), [SECURITY.md](SECURITY.md), and [DEPLOYMENT.md](DEPLOYMENT.md).
...
ReClass is a modular monolith, not a microservice system. SvelteKit pages, loads, actions, middleware, and partial domain services deploy together; five Edge Functions isolate payment and SMS integration work. Durable state is held in a shared-schema, row-per-tenant PostgreSQL database.
...
Latest observed results on 2026-07-22:

| Check | Result |
|---|---|
| `npm run test` | Pass: 8 files, 81 tests |
| `npm run check` | Pass with 0 errors and 11 warnings in 5 files; warnings are accessibility/HTML issues |
| `npm run build` | Pass using `@sveltejs/adapter-vercel`, with the same 11 warnings and adapter dependency-location warnings |
