...
This is the authoritative delivery sequence derived from [AUDIT-2026-08.md](AUDIT-2026-08.md). Phases are gated: the team should not start scale or feature expansion work while an earlier phase has an open release-blocking criterion.

| Phase | Timing | Objective | Primary owners | Effort | Exit gate |
|---|---|---|---|---:|---|
| 0. Contain and baseline | Days 0–2 | Freeze unsafe production changes, capture hosted schema/backups/configuration, assign owners | Tech lead, DBA, DevOps, security | 2–4 days | Evidence pack exists; no unknown production mutation path; P0 tickets owned |
| 1. Database and tenant boundary | Weeks 1–3 | Make migrations replayable and tenant isolation enforceable | DBA, backend, security | 3–5 weeks | Clean reset twice, upgrade rehearsal, generated types match, cross-tenant negative suite passes |
| 2. Financial and workflow correctness | Weeks 3–6 | Protect payments, waivers, academic writes, messaging, and imports | Backend, product, QA | 3–5 weeks | Transactional/idempotent flows pass concurrency and failure tests; audit events exist |
| 3. Release engineering and operations | Weeks 5–8 | Make deployment, monitoring, secrets, rollback, and recovery repeatable | DevOps, SRE, security | 3–5 weeks | One supported deployment path, CI gates, readiness, alerts, rollback and restore drills pass |
| 4. Scale foundation and UX quality | Months 2–4 | Remove silent truncation and memory bottlenecks; validate accessibility and localization | Backend, frontend, SRE, QA | 6–10 weeks | Load/soak budgets, cursor pagination, SQL aggregates, axe/manual WCAG sign-off |
| 5. Controlled product expansion | Months 4–9 | Complete school operations, tenant lifecycle, analytics, and premium foundations | Product, engineering, support, finance | 3–6 months | Pilot UAT, plan entitlements, governed metrics, retention/export workflows |
| 6. Intelligent platform evolution | Months 9–12+ | Add constrained AI and selectively split services only when measured | Product, AI, platform, security | Multi-quarter | AI safety evaluation, opt-in pilot, measurable scale threshold, reversible rollout |

### Phase 0 — Contain and baseline

Actions:

- Freeze ad hoc production SQL and obtain a schema/migration ledger snapshot.
- Confirm backup/PITR status and create a verified non-production restore target.
- Inventory Edge Function versions, cron jobs, Vault secrets, callback secrets, provider credentials, deployment targets, and owners.
- Require review for every service-role query and sensitive mutation.
- Create one tracking board with P0/P1 issues, evidence links, owner, due date, and rollback plan.

Deliverable: a signed baseline pack containing schema, deployment, secrets/configuration presence, backup status, and risk ownership.

### Phase 1 — Database and tenant boundary

Actions:

- Repair migration ordering/helper dependencies with forward-only migrations; do not edit applied migrations.
- Choose one coherent authorization model: user-scoped RLS for normal requests plus narrow privileged RPCs is preferred.
- Add composite tenant constraints to same-tenant relationships.
- Replace implicit/duplicated tenant filtering with a typed data-access boundary.
- Regenerate database types and fail CI on schema drift.
- Execute role-based cross-tenant denial tests against a real database.

Deliverable: reproducible database and a demonstrated tenant boundary, not merely source-level predicates.

### Phase 2 — Financial and workflow correctness

Actions:

- Convert academic result replacement, message append, bulk invoice generation, payment settlement, and waiver operations to transactional RPCs where multiple writes form one invariant.
- Enforce payment amount/phone/check-out binding, duplicate active-checkout prevention, expiry, replay handling, and provider reconciliation.
- Add notification leases, idempotency keys, retry scheduling, dead-letter handling, and crash recovery.
- Add strict import byte/row/schema validation and CSV formula neutralization.
- Make impersonation use a mandatory dedicated secret, validated targets, secure cookies, revocation, and audit events.
- Implement append-only audit coverage for roles, credentials, impersonation, imports, academic, finance, payroll, and provider decisions.

Deliverable: failure-safe financial and administrative workflows with traceable business effects.

### Phase 3 — Release engineering and operations

Actions:

- Select Vercel or container deployment as the supported path and remove adapter mutation from Docker builds.
- Add migration reset/upgrade, generated-type, Edge Function, Playwright, accessibility, SCA/SAST/secret, and coverage gates.
- Make production secrets mandatory, rotated, and never logged; separate platform and tenant secrets.
- Add liveness/readiness endpoints, structured redacted logs, metrics, traces, queue lag, provider failure, and payment reconciliation alerts.
- Define RPO/RTO, rollback triggers, migration compatibility rules, incident ownership, and change approval.
- Execute staging rollback, provider outage, queue recovery, and database restore drills.

Deliverable: a release can be promoted, observed, rolled back, and recovered by someone other than the original implementer.

### Phase 4 — Scale foundation and UX quality

Actions:

- Replace unbounded/offset reads with cursor pagination and move dashboard/report aggregates into SQL.
- Add query-plan review, tenant-aware indexes, async large exports, bounded imports, caching for stable metadata, and provider timeouts/circuit breakers.
- Establish p95/p99 performance budgets and run load/soak tests at production-like tenant cardinality.
- Complete keyboard, screen-reader, contrast, reflow, focus, and low-bandwidth validation on critical flows.
- Complete English/Swahili translation coverage and make notification state account-backed.

Deliverable: measured scale behavior and an accessible, resilient critical user journey.

### Phase 5 — Controlled product expansion

Actions:

- Add guided tenant onboarding, suspension, export, deletion, retention/legal hold, and support tooling.
- Finish consent/STOP handling, announcements, scheduled reports, holiday/calendar exceptions, offline attendance, and conflict resolution.
- Define governed metrics with freshness, denominator, and data-quality indicators.
- Add server-enforced plans, metering, billing evidence, dunning, grace periods, and entitlement audit.
- Pilot premium features with a small set of tenants and explicit UAT criteria.

Deliverable: commercially usable product expansion without weakening tenant, financial, or operational guarantees.

### Phase 6 — Intelligent platform evolution

Actions:

- Begin with low-risk AI: draft summaries, anomaly triage, import suggestions, and natural-language report filters.
- Require tenant opt-in, PII minimization, provider no-training terms, isolated retrieval/cache/logging, prompt-injection testing, human confirmation, source display, cost controls, and kill switches.
- Keep AI out of autonomous grades, attendance, payments, waivers, payroll, access, messaging, and student-welfare decisions.
- Reassess service extraction or database partitioning only after telemetry proves a bottleneck and ownership boundary.

Deliverable: reversible, governed intelligence and scale architecture justified by measured demand.

## Phase Decision Rules

- A phase may be marked complete only when its exit gate has executable evidence attached.
- Security, tenant isolation, financial correctness, and recoverability outrank feature delivery.
- No microservice, sharding, or AI work is a substitute for missing authorization, transactions, observability, or backups.
- Breaking schema changes use expand-and-contract, bounded backfills, compatibility windows, and a tested rollback/forward path.
- Every phase ends with a demo, evidence review, updated runbook, and explicit residual-risk decision.
