-- eShule school-operations foundation: lifecycle, calendar and teacher reminders.
-- Discipline is intentionally out of the current eShule scope.
...
CREATE TABLE IF NOT EXISTS public.student_lifecycle_events (
  ...
  from_class_id uuid REFERENCES public.sis_classes(id) ON DELETE RESTRICT,
  to_class_id uuid REFERENCES public.sis_classes(id) ON DELETE RESTRICT,
  notes text,
...
CREATE TABLE IF NOT EXISTS public.school_calendar_events (
  ...
  tenant_id uuid NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
  title text NOT NULL,
  event_type text NOT NULL DEFAULT 'school' CHECK (event_type IN ('school','exam','meeting','holiday','activity','deadline','other')),
