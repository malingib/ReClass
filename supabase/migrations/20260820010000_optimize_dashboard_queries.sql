...
BEGIN
    ...
    WITH student_counts AS (
        SELECT COUNT(*)::BIGINT AS total_students_count FROM students WHERE tenant_id = p_tenant_id AND deleted_at IS NULL
    ),
    teacher_counts AS (
        SELECT COUNT(*)::BIGINT AS total_teachers_count FROM teachers WHERE tenant_id = p_tenant_id AND deleted_at IS NULL
    ),
    session_counts AS (
        SELECT COUNT(*)::BIGINT AS total_sessions_count FROM sessions WHERE tenant_id = p_tenant_id AND active = true
    ),
    due_occurrences AS (
        SELECT COUNT(*)::BIGINT AS count FROM session_occurrences WHERE tenant_id = p_tenant_id AND occurs_on BETWEEN p_since_date AND p_today_date AND status != 'cancelled'
    ),
    delivered_attendance AS (
        SELECT COUNT(*)::BIGINT AS count
        FROM teacher_attendance ta
        JOIN session_occurrences so ON ta.session_occurrence_id = so.id
        WHERE ta.tenant_id = p_tenant_id AND ta.approval_status = 'approved' AND ta.status IN ('present', 'late') AND so.occurs_on BETWEEN p_since_date AND p_today_date
    ),
    pending_attendance AS (
        SELECT COUNT(*)::BIGINT AS count FROM teacher_attendance WHERE tenant_id = p_tenant_id AND approval_status = 'pending' AND deleted_at IS NULL
    )
    SELECT
...
BEGIN
    ...
    WITH daily_attendance AS (
        ...
        LEFT JOIN teacher_attendance ta ON so.id = ta.session_occurrence_id
...
-- SIS stats view aligned to the canonical SIS tables and their actual status columns.
