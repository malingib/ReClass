-- A committee member with the explicit approval right approves teacher attendance.
...
declare a public.teacher_attendance; ok boolean; begin
 ...
 select exists(select 1 from public.remedial_committee_role_assignments ra join public.remedial_committee_rights rr on rr.assignment_id=ra.id where ra.tenant_id=a.tenant_id and ra.teacher_id=auth.uid() and ra.active and rr.right_code='view_committee' and rr.granted) into ok;
