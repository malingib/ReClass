...
> Current-state note (2026-08-01): deployment remains release-blocked pending the exit criteria in [AUDIT-2026-08.md](AUDIT-2026-08.md). This file is the detailed operational deployment reference.
...
| `MPESA_CALLBACK_SECRET` | production mandatory | Callback requests fail closed when absent or mismatched (`mpesa-callback/index.ts`). Confirm Safaricom/proxy can supply `x-callback-secret` before enabling payments. |
