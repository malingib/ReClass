...
describe('rateLimitedHeaders', () => {
  it('reports limit as remaining + 1 when allowed', () => {
    const h = rateLimitedHeaders({ allowed: true, limit: 10, remaining: 4, resetIn: 60_000 });
    expect(h['X-RateLimit-Limit']).toBe('10');
    expect(h['X-RateLimit-Remaining']).toBe('4');
    expect(h['X-RateLimit-Reset']).toBe('60');
  });

  it('reports the configured limit when blocked', () => {
    const h = rateLimitedHeaders({ allowed: false, limit: 5, remaining: 0, resetIn: 30_000 });
...
describe('checkRateLimit', () => {
  it('returns RPC result when the DB responds', async () => {
    ...
    expect(r.limit).toBe(RATE_LIMITS.login.max);
    expect(r.resetIn).toBe(12_000);
  });

  it('fails closed for login when the DB errors', async () => {
  ...
  it('fails closed for STK when the RPC throws', async () => {
    const srv = { rpc: async () => { throw new Error('boom'); } } as never;
    const r = await checkRateLimit(srv, 'k', 'stk');
    expect(r.allowed).toBe(false);
    expect(r.remaining).toBe(0);
  });

  it('keeps the general namespace available during a limiter outage', async () => {
    const srv = { rpc: async () => { throw new Error('boom'); } } as never;
