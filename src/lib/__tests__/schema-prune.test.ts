...
describe('schema prune regression guard (invoice lifecycle removed)', () => {
  ...
  it('no migration after 000003 re-adds an invoice_id column to payments', () => {
    const later = files.filter((f) => f > '20260731000003');
    for (const f of later) {
      expect(sql(f), `migration ${f} must not re-add payments.invoice_id`).not.toMatch(
        /alter table[^;]*\bpayments\b[^;]*add column[^;]*invoice_id/i
      );
    }
  });
});
