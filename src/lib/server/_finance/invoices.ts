import { countRecords, flattenStudentName } from '../_platform/query';
...
export async function getInvoicesByTenant(
  ...
  const [{ count }, { data }] = await Promise.all([
    (() => {
      let q = sb.from('invoices').select('*', { count: 'exact', head: true }).eq('tenant_id', tenantId).is('deleted_at', null);
      if (opts.filters) q = opts.filters(q);
      return q;
    })(),
    (() => {
      let q = sb.from('invoices').select(INVOICE_SELECT).eq('tenant_id', tenantId).is('deleted_at', null);
      if (opts.filters) q = opts.filters(q);
...
export async function getInvoiceCounts(sb: App.Locals['srv'], tenantId: string) {
  const [unpaid, paid, total] = await Promise.all([
    countRecords(sb, 'invoices', tenantId, q => q.eq('status', 'unpaid').is('deleted_at', null)),
    countRecords(sb, 'invoices', tenantId, q => q.eq('status', 'paid').is('deleted_at', null)),
    countRecords(sb, 'invoices', tenantId, q => q.is('deleted_at', null)),
  ]);
