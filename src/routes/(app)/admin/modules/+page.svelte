...
<div class="mx-auto max-w-5xl py-8">
  ...
  <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
    {#each suiteModules.filter(m => m.status === 'available') as m (m.id)}
      <a href={m.href!}
        class="group relative overflow-hidden rounded-2xl border border-border/60 bg-white p-6 shadow-card transition-all duration-200 hover:-translate-y-1 hover:shadow-card-hov"
      >
