...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  const ids = (teacherRoles ?? []).map((r: any) => r.user_id);
  const { data: teachers } = ids.length ? await db.from('profiles').select('id,full_name,phone').in('id', ids).order('full_name') : { data: [] };
...
export const actions = {
  ...
  assignOperator: async ({ locals, request }) => {
    ...
    const { error: validationError } = await locals.srv.rpc('validate_reclass_paybill_role_operator', { p_tenant_id: tenantId, p_role_assignment_id: v.data.role_assignment_id, p_operator_role: v.data.operator_role, p_approval_level: level });
    if (validationError) return fail(400, { message: validationError.message });
    const { error } = await locals.srv.from('remedial_paybill_operators').upsert({ tenant_id: tenantId, role_assignment_id: v.data.role_assignment_id, committee_member_id: null, operator_role: v.data.operator_role, approval_level: level, active: true }, { onConflict: 'tenant_id,committee_member_id,operator_role,approval_level' });
    if (error) return fail(500, { message: error.message });
