...
export const load: PageServerLoad = async ({ locals }) => {
  const { tenantId } = requireTenantRole(locals, 'school_admin', 'super_admin', 'principal'); const db: any = locals.srv as any;
...
export const actions = {
  assignRole: async ({ locals, request }) => {
    ...
    const { error: validationError } = await (locals.srv as any).rpc('validate_reclass_committee_role_assignment', { p_tenant_id: tenantId, p_role_id: v.data.role_id, p_teacher_id: v.data.teacher_id }); if (validationError) return fail(400, { message: validationError.message });
    const { error } = await (locals.srv as any).from('remedial_committee_role_assignments').upsert({ tenant_id: tenantId, role_id: v.data.role_id, teacher_id: v.data.teacher_id, active: true, assigned_by: locals.user?.id ?? null }, { onConflict: 'tenant_id,role_id,teacher_id' }); if (error) return fail(500, { message: error.message }); return { success: true, message: 'Committee role assigned to teacher.' };
  },
  assignOperator: async ({ locals, request }) => {
    ...
    const { error: validationError } = await (locals.srv as any).rpc('validate_reclass_paybill_role_operator', { p_tenant_id: tenantId, p_role_assignment_id: v.data.role_assignment_id, p_operator_role: v.data.operator_role, p_approval_level: level }); if (validationError) return fail(400, { message: validationError.message });
    const { data: existing } = await (locals.srv as any).from('remedial_paybill_operators').select('id').eq('tenant_id', tenantId).eq('role_assignment_id', v.data.role_assignment_id).eq('operator_role', v.data.operator_role).eq('approval_level', level).maybeSingle();
    ...
    const result = existing ? await (locals.srv as any).from('remedial_paybill_operators').update(payload).eq('id', existing.id).eq('tenant_id', tenantId) : await (locals.srv as any).from('remedial_paybill_operators').insert(payload); if (result.error) return fail(500, { message: result.error.message }); return { success: true, message: 'PayBill rights assigned to the teacher holding that committee role.' };
  },
  grantRights: async ({ locals, request }) => {
    ...
    const { data: assignment } = await (locals.srv as any).from('remedial_committee_role_assignments').select('id').eq('id', v.data.assignment_id).eq('tenant_id', tenantId).maybeSingle(); if (!assignment) return fail(404, { message: 'Committee assignment not found.' });
    const { error: deleteError } = await (locals.srv as any).from('remedial_committee_rights').delete().eq('assignment_id', v.data.assignment_id).eq('tenant_id', tenantId); if (deleteError) return fail(500, { message: deleteError.message });
    if (selected.length) { const { error } = await (locals.srv as any).from('remedial_committee_rights').insert(selected.map((right_code) => ({ tenant_id: tenantId, assignment_id: v.data.assignment_id, right_code, granted: true, granted_by: locals.user?.id ?? null }))); if (error) return fail(500, { message: error.message }); }
  ...
  saveSettings: async ({ locals, request }) => {
    ...
    const { error } = await (locals.srv as any).from('remedial_paybill_settings').upsert({ tenant_id: tenantId, paybill_number: v.data.paybill_number || null, account_prefix: v.data.account_prefix || null, approval_levels: v.data.approval_levels, minimum_web_operators: 2, maker_checker_required: true, initiator_may_approve_own_transaction: false, updated_by: locals.user?.id ?? null }, { onConflict: 'tenant_id' }); if (error) return fail(500, { message: error.message }); return { success: true, message: 'PayBill governance settings saved.' };
  },
  deactivate: async ({ locals, request }) => {
    const { tenantId } = requireTenantRole(locals, 'school_admin', 'super_admin', 'principal'); const v = parseForm(idSchema, await request.formData()); if (!v.success) return fail(400, { message: 'Assignment not found.' }); await (locals.srv as any).from('remedial_paybill_operators').update({ active: false, effective_to: new Date().toISOString() }).eq('id', v.data.id).eq('tenant_id', tenantId); return { success: true, message: 'Operator assignment deactivated.' };
  },
} satisfies Actions;
