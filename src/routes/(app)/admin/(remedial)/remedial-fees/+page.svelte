<script lang="ts">
  ...
  function openCreate() { formData = {}; editingFee = null; showCreate = true; }
  function openEdit(f: Fee) {
    editingFee = f;
    formData = { id: f.id, name: f.name, amount: f.amount, due_date: f.due_date ?? '', term: f.term ?? '' };
    showCreate = true;
  }
  function openDelete(f: Fee) { deletingFee = f; }
</script>

<DashboardContent title="Remedial fee structures" subtitle="Fees collected from parents via M-Pesa paybill for remedial classes">
  {#snippet headerActions()}
    <button onclick={openCreate} class="rounded-full bg-brand-600 px-4 py-2 text-xs font-medium text-white hover:bg-brand-700 flex items-center gap-1.5">
      <Plus class="h-3.5 w-3.5" /> Add Remedial Fee
    </button>
  {/snippet}
  ...
  <Dialog.Portal>
    ...
    <Dialog.Content class="fixed left-1/2 top-1/2 z-50 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-border bg-white p-0 shadow-elevated">
      <div class="flex items-center justify-between border-b border-border px-6 py-4">
        <Dialog.Title class="text-base font-semibold text-ink-900">{editingFee ? 'Edit Remedial Fee' : 'Add Remedial Fee'}</Dialog.Title>
  ...
  <Dialog.Portal>
    ...
    <Dialog.Content class="fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-border bg-white p-0 shadow-elevated">
      <div class="px-6 py-5 text-center">
        ...
        <h3 class="text-base font-semibold text-ink-900">Delete Remedial Fee</h3>
