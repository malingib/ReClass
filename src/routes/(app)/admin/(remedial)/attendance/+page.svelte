...
<DashboardContent title="Teacher attendance" subtitle="Review remedial delivery records before they become payroll evidence.">
  <div class="space-y-6">
    ...
    {#if approved.length > 0}<div class="ui-status ui-status-success"><CheckCircle2 class="h-4 w-4" /><span>Approved attendance remains available as payroll evidence. <a href="/admin/payroll" class="font-semibold underline">Open payroll</a></span></div>{/if}
  </div>
</DashboardContent>