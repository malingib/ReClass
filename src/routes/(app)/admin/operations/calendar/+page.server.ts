...
export const actions = {
  create: async ({ locals, request }) => {
    const { tenantId, user } = requireTenantRole(locals, 'school_admin', 'principal', 'super_admin');
    ...
    const { error } = await locals.srv.from('school_calendar_events').insert({ tenant_id: tenantId, title, event_type: String(f.get('event_type') || 'school'), starts_at: new Date(starts).toISOString(), ends_at: ends ? new Date(ends).toISOString() : null, all_day: f.get('all_day') === 'on', audience: String(f.get('audience') || 'staff'), location: String(f.get('location') || '') || null, description: String(f.get('description') || '') || null, created_by: user.id });
