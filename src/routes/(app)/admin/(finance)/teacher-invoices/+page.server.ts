...
import { getTeacherInvoices, getTeachersList, generateTeacherInvoices } from '$lib/server/_finance/teacher-invoices';
...
export const actions = {
  create: async ({ locals, request }) => {
    ...
    const { data: teacher } = await locals.srv.from('teachers').select('id').eq('id', v.data.teacher_id).eq('tenant_id', tenantId).is('deleted_at', null).maybeSingle();
