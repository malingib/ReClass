...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  const [
    ...
    { data: rs }, { data: ri }, { data: ta }, { data: occ }, { data: sessionsByStatus },
    { data: sum },
  ] = await Promise.all([
    ...
    sb.from('sessions').select('*', { count: 'exact', head: true }).eq('tenant_id', tid),
  ...
  // "Session coverage" = remedial sessions scheduled per day over the last 14 days.
  // Built from session_occurrences (the real session data), not attendance — the
  // card is titled "session coverage" and attendance may legitimately be empty.
  const trend: { label: string; value: number }[] = [];
  for (let i = 13; i >= 0; i--) {
    const d = new Date(Date.now() - i * 864e5);
    const dayStr = d.toISOString().slice(0, 10);
    const daySessions = (occ || []).filter((s: any) => (s.occurs_on || '').slice(0, 10) === dayStr);
