...
export const load: PageServerLoad = async ({ locals, url }) => {
  ...
  const db: any = locals.srv as any;
  const [{ data: attendance }, { data: payroll }, { data: payments }, { data: invoices }] = await Promise.all([
    db.from('v_teacher_attendance_daily').select('day,attended,absent,total').eq('tenant_id', tenantId).gte('day', since).order('day'),
    db.from('v_payroll_weekly').select('period_start,period_end,teacher_count,payroll_total,paid_teachers,confirmed_teachers').eq('tenant_id', tenantId).order('period_start'),
    db.from('payments').select('created_at,amount,status,method').eq('tenant_id', tenantId).gte('created_at', `${since}T00:00:00`).order('created_at'),
    db.from('invoices').select('amount_due,amount_paid,status').eq('tenant_id', tenantId).is('deleted_at', null),
  ]);
