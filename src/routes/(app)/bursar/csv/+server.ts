...
export const GET: RequestHandler = async ({ locals }) => {
  ...
  if (!payments) error(500, 'Failed to fetch payment data');

  const headers = ['Student Name', 'Admission No', 'Grade', 'Fee', 'Amount (KES)', 'Channel', 'Reference', 'Receipt No', 'Created At'];
  ...
  return csvResponse(headers, rows, `all-receipts-${new Date().toISOString().slice(0, 10)}.csv`);
};
