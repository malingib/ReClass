...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  const [{ data: timetable }, { data: occurrences }, { data: announcements }, { data: homeroomClasses }, { data: calendarEvents }, { data: tasks }] = await Promise.all([
    ...
    locals.srv.from('school_calendar_events').select('id, title, event_type, starts_at, ends_at, all_day, audience, location').eq('tenant_id', tenantId).in('audience', ['teachers', 'all', 'staff']).gte('starts_at', new Date().toISOString()).lte('starts_at', calendarThrough).order('starts_at').limit(8),
    locals.srv.from('teacher_tasks').select('id, title, description, due_at, priority, status, reminder_minutes').eq('tenant_id', tenantId).eq('teacher_id', teacher.id).in('status', ['open', 'in_progress']).order('due_at').limit(8),
  ]);
  ...
  })) as Array<{ id: string; occurs_on: string; class: string | null; start_time: string; end_time: string; room: string | null; status: string; subject: string; attendance: { id: string; status: string; marked_at: string | null; approval_status: string; review_note: string | null } | null }>;

  return { capabilities: caps, teacherType: teacher.teacher_type, canRemedial, canSis, stats: { sessions: timetable?.length ?? 0, pending: delivery.filter((item) => item.attendance?.approval_status === 'pending').length }, homeroomCount: homeroomClasses?.length ?? 0, teacher, timetable: timetable ?? [], occurrences: delivery, announcements: announcements ?? [], calendarEvents: calendarEvents ?? [], tasks: tasks ?? [] };
};

export const actions = {
  mark: async ({ locals, request }) => {
    const { user, tenantId, teacher } = await getTeacherOwnership(locals);
    if (!hasCapability(locals.role, teacher.teacher_type, 'remedial:attendance_mark')) return fail(403, { error: 'You do not have permission to mark remedial attendance' });
    ...
    const { data, error } = await locals.srv.rpc('mark_own_teacher_attendance', { p_tenant_id: tenantId, p_teacher_id: teacher.id, p_profile_id: user.id, p_occurrence_id: occurrenceId, p_status: status });
